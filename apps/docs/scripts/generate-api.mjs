...
function sanitizeSignature(signature) {
  ...
  for (const entry of apiPackages) {
    ...
    clean = clean.replace(
      new RegExp(`import\\(["']${escapedRoot}/(?:dist|src)/index["']\\)`, "g"),
      `import("${entry.name}")`,
    );
  }
  return clean;
...
function declarationSignature(checker, symbol, declaration) {
  ...
  if (calls.length > 0) {
    return sanitizeSignature(calls
      .map((signature) => `${symbol.name}${checker.signatureToString(
        signature,
        declaration,
  ...
  return sanitizeSignature(`${symbol.name}: ${checker.typeToString(type, declaration, ts.TypeFormatFlags.NoTruncation)}`);
}
...
function publicSymbol(checker, exportedSymbol, packageEntry) {
  ...
  if (!declaration) throw new Error(`${packageEntry.name}: ${exportedSymbol.name} has no declaration.`);
  ...
  return {
    ...
    description: documentation || `Public ${category.slice(0, -1)} exported by ${packageEntry.name}.`,
    source,
    line,
...
function createCatalog() {
  ...
  return Object.fromEntries(entries.map((entry) => {
    ...
    if (!sourceFile || !moduleSymbol) throw new Error(`Cannot load public entrypoint for ${entry.name}.`);
...
if (checkOnly) {
  ...
  if (current !== serialized) {
    console.error("API catalog is stale. Run `pnpm --filter docs content:api:write`.");
    process.exit(1);
  }
  ...
  console.log(`Generated API catalog at ${relative(repositoryRoot, outputPath)}.`);
}
